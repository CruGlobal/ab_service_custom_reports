...
class TeamRcFyOptions {
         ...
         $mcc.__onChange = $mcc.attachEvent("onChange", async () => {
            ...
            mccVal.split(",").forEach((mcc) => {
               ($$(reportElementId()).__mccRcs || []).forEach((item) => {
                  if (item.mcc == mcc && rcs.indexOf(item.rc) < 0)
                     rcs.push(item.rc);
               });
            });
   ...
   async _loadOptions() {
      const ids = this.ids;
      const isCoreUser =
         AB.Account.roles((r) => (r.uuid || r) == ids.coreFinanceRoleId)
            .length > 0;
      ...
      this._defineOptions(
         ...
         "FY Per",
         false,
         this._options.customOptionLabel,
      );
      this._defineOptions(
         ids.endViewId,
         (months && months.data) || [],
   ...
   async _defineRcOptions() {
      ...
      else rcs = await this._pullMyRC(teamList, this._options.filterRC);
   ...
   _defineOptions(
      ...
      if (isSorted) options = options.sort(this._sort);
      ...
      $$(webixId).define(
         ...
         options.map((opt) => {
            return {
               id: opt,
               value: fnCustomLabel ? `${opt} - [${fnCustomLabel(opt)}]` : opt,
            };
         }),
      );
   }
   ...
   async _pullAllRC(teams = [], filter = null) {
      ...
      if (filter) where.rules.push(filter);
